...
import com.benidict.common_ui.navigation.route.CartScreeRoute

fun NavGraphBuilder.cartNavGraph(navRoute: @Composable (NavBackStackEntry, route: Any) -> Unit) {
    navigation<CartGraph>(startDestination = CartScreeRoute) {
        composable<CartScreeRoute> {
            navRoute(it, CartScreeRoute)
        }
    }
}